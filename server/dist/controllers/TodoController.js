...
const listTodos = async (req, res) => {
    try {
        const userId = req?.user?._id || req.query.userId; // fallback si pas d'auth middleware
        if (!userId)
            return res.status(401).json({ error: "Unauthorized" });
        const { q, status = "open", page = "1", limit = "20", dateFrom, dateTo, sort = "dueAsc" } = req.query;
        ...
        if (dateFrom || dateTo) {
            filter.dueAt = {};
            if (dateFrom)
                filter.dueAt.$gte = new Date(`${dateFrom}T00:00:00.000Z`);
            if (dateTo)
                filter.dueAt.$lte = new Date(`${dateTo}T23:59:59.999Z`);
        }
        // Tri
        const sortMap = {
        ...
        const [items, total] = await Promise.all([
            TodoModel_1.default.find(filter).sort(sortObj).skip(skip).limit(limitNum).lean(),
            TodoModel_1.default.countDocuments(filter),
        ]);
...
const createTodo = async (req, res) => {
    try {
        ...
        const doc = await TodoModel_1.default.create({
...
const updateTodo = async (req, res) => {
    try {
        ...
        const doc = await TodoModel_1.default.findOneAndUpdate({ _id: id, createdBy: userId }, { $set: patch }, { new: true });
        if (!doc)
...
const deleteTodo = async (req, res) => {
    try {
        ...
        if (!userId)
            return res.status(401).json({ error: "Unauthorized" });
        const { id } = req.params;
        const doc = await TodoModel_1.default.findOneAndDelete({ _id: id, createdBy: userId });
        if (!doc)
            return res.status(404).json({ error: "Not found" });
